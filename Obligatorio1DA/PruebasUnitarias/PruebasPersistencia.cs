...
namespace PruebasUnitarias
{
    ...
    public class PruebasPersistencia
    {
        ...
        public Alumno ObtenerAlumnoDb(int id)
        {
            Alumno alumno = contextoDb.Alumnos.FirstOrDefault(st => st.Id == id);
            if (alumno == null)
                throw new ArgumentException("La entidad no existe");
            return alumno;
        }
        ...
        public void ProbarBajarAlumnoBaseDatos()
        {
            ...
            if (alumnoBaseDatos != null)
            {
                idAlumno = alumnoDb[0].Id;
                contextoDb = new ContextoDb();
                Alumno alumno = ObtenerAlumnoDb(idAlumno);
                contextoDb.Alumnos.Attach(alumno);
                contextoDb.Alumnos.Remove(alumno);
                contextoDb.SaveChanges();
            }
            alumnoDb = contextoDb.Alumnos.SqlQuery("Select * from Alumnoes where Ci = '8883333'").ToList();
            Assert.AreNotEqual(alumnoDb.Count, contextoDb.Alumnos.Count());
        }
        public void SetUp()
        {
